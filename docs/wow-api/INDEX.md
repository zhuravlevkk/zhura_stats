# WoW API Index (build 12.0.5.67823)

Generated from a `wow-ui-source` clone on 2026-06-06 05:46 UTC.
Systems: 567 | Functions/events: 9866 | Conditionally-secret entries: 286

Regenerate with `python scripts/gen_api_index.py`.

## How to use this

- Find a function: search this file or `api-index.json` for its name.
- The **system** maps to `Blizzard_APIDocumentationGenerated/<System>Documentation.lua` in the clone -- open that file only when you need more than the digest provides.
- **`SECRET-VALUES.md`** -- functions whose return becomes Secret, grouped by condition, with Blizzard predicate descriptions.
- **`ADDON-RESTRICTIONS.md`** -- `AddOnRestrictionType` / `AddOnRestrictionState` enums and key restriction events/APIs.
- **`api-index.json`** -- machine-readable digest including predicate docs, restriction enums, and Arguments/Returns signatures.
- If an entry appears in `SECRET-VALUES.md`, guard with `issecretvalue()` before comparing, doing arithmetic, or using it as a table key.

## Systems

- **AbbreviateConfig** (3) -- `AbbreviateConfigAPIDocumentation.lua`
- **AbbreviatedNumberFormatter** (7) -- `AbbreviatedNumberFormatterAPIDocumentation.lua`
- **AccessibilityOptions** (1) -- `AccessibilityOptionsDocumentation.lua`
- **AccountConstants** (11) -- `AccountConstantsDocumentation.lua`
- **AccountInfo** (4) -- `AccountInfoDocumentation.lua`
- **AccountStore** (20) -- `AccountStoreDocumentation.lua`
- **AchievementInfo** (18) -- `AchievementInfoDocumentation.lua`
- **AchievementTelemetry** (4) -- `AchievementTelemetryDocumentation.lua`
- **Action** (1) -- `ActionDocumentation.lua`
- **ActionBarFrame** (86) -- 5 secret -- `ActionBarFrameDocumentation.lua`
- **ActionBarShared** (2) -- `ActionBarSharedDocumentation.lua`
- **AddOnProfiler** (15) -- `AddOnProfilerDocumentation.lua`
- **AddOnProfilerConstants** (2) -- `AddOnProfilerConstantsDocumentation.lua`
- **AddOns** (37) -- `AddOnsDocumentation.lua`
- **AdventureJournal** (15) -- `AdventureJournalDocumentation.lua`
- **AdventureMap** (9) -- `AdventureMapDocumentation.lua`
- **AlliedRacesFrame** (7) -- `AlliedRacesFrameDocumentation.lua`
- **AnimaDiversionUI** (15) -- `AnimaDiversionUIDocumentation.lua`
- **AppearanceSource** (1) -- `AppearanceSourceDocumentation.lua`
- **ArdenwealdGardening** (4) -- `ArdenwealdGardeningDocumentation.lua`
- **AreaPoiInfo** (11) -- `AreaPoiInfoDocumentation.lua`
- **ArrowCalloutConstants** (2) -- `ArrowCalloutConstantsDocumentation.lua`
- **ArtifactUI** (80) -- `ArtifactUIDocumentation.lua`
- **AssistedCombat** (6) -- `AssistedCombatDocumentation.lua`
- **AsyncAssistActionsConstants** (1) -- `AsyncAssistActionsConstantsDocumentation.lua`
- **AuctionHouse** (149) -- `AuctionHouseDocumentation.lua`
- **AuctionHouseConstants** (1) -- `AuctionHouseConstantsDocumentation.lua`
- **AuctionHouseEnums** (8) -- `AuctionHouseEnumsDocumentation.lua`
- **AutoComplete** (6) -- `AutoCompleteDocumentation.lua`
- **AutoCompleteShared** (2) -- `AutoCompleteSharedDocumentation.lua`
- **AzeriteConstants** (1) -- `AzeriteConstantsDocumentation.lua`
- **AzeriteEmpoweredItem** (26) -- `AzeriteEmpoweredItemDocumentation.lua`
- **AzeriteEssence** (32) -- `AzeriteEssenceDocumentation.lua`
- **AzeriteItem** (15) -- `AzeriteItemDocumentation.lua`
- **BNetOutage** (3) -- `BNetOutageDocumentation.lua`
- **BagConstants** (2) -- `BagConstantsDocumentation.lua`
- **BagIndexConstants** (2) -- `BagIndexConstantsDocumentation.lua`
- **Bank** (34) -- `BankDocumentation.lua`
- **BarberShop** (37) -- `BarberShopDocumentation.lua`
- **BarberShopInternal** (2) -- `BarberShopInternalDocumentation.lua`
- **Base** (1) -- `BaseDocumentation.lua`
- **BattleNet** (17) -- `BattleNetDocumentation.lua`
- **BattlePet** (27) -- `BattlePetDocumentation.lua`
- **BattlePetConstants** (23) -- `BattlePetConstantsDocumentation.lua`
- **BattlepayConstants** (7) -- `BattlepayConstantsDocumentation.lua`
- **BehavioralMessaging** (3) -- `BehavioralMessagingDocumentation.lua`
- **BlackMarketInfo** (8) -- `BlackMarketInfoDocumentation.lua`
- **BountyShared** (1) -- `BountySharedDocumentation.lua`
- **Browser** (6) -- `BrowserDocumentation.lua`
- **Build** (12) -- `BuildDocumentation.lua`
- **CVar** (15) -- `CVarDocumentation.lua`
- **Calendar** (116) -- 4 secret -- `CalendarDocumentation.lua`
- **CalendarConstants** (19) -- `CalendarConstantsDocumentation.lua`
- **Camera** (3) -- `CameraDocumentation.lua`
- **CameraConstants** (1) -- `CameraConstantsDocumentation.lua`
- **CatalogShop** (70) -- `CatalogShopDocumentation.lua`
- **CatalogShopConstants** (1) -- `CatalogShopConstantsDocumentation.lua`
- **ChallengeModeInfo** (51) -- `ChallengeModeInfoDocumentation.lua`
- **CharacterCreationConstants** (2) -- `CharacterCreationConstantsDocumentation.lua`
- **CharacterCustomizationShared** (9) -- `CharacterCustomizationSharedDocumentation.lua`
- **CharacterSelectionConstants** (8) -- `CharacterSelectionConstantsDocumentation.lua`
- **CharacterServicesConstants** (1) -- `CharacterServicesConstantsDocumentation.lua`
- **ChatBubbles** (2) -- `ChatBubblesDocumentation.lua`
- **ChatConstants** (17) -- `ChatConstantsDocumentation.lua`
- **ChatInfo** (148) -- 59 secret -- `ChatInfoDocumentation.lua`
- **ChatShared** (1) -- `ChatSharedDocumentation.lua`
- **ChromieTimeUI** (6) -- `ChromieTimeUIDocumentation.lua`
- **Cinematic** (14) -- `CinematicDocumentation.lua`
- **CinematicConstants** (1) -- `CinematicConstantsDocumentation.lua`
- **CinematicList** (3) -- `CinematicListDocumentation.lua`
- **ClassColor** (2) -- `ClassColorDocumentation.lua`
- **ClassTalents** (42) -- `ClassTalentsDocumentation.lua`
- **ClassTrial** (3) -- `ClassTrialDocumentation.lua`
- **ClickBindings** (13) -- `ClickBindingsDocumentation.lua`
- **ClickBindingsConstants** (3) -- `ClickBindingsConstantsDocumentation.lua`
- **Client** (11) -- `ClientDocumentation.lua`
- **ClientScene** (4) -- `ClientSceneDocumentation.lua`
- **ClientSettings** (1) -- `ClientSettingsDocumentation.lua`
- **Club** (139) -- 13 secret -- `ClubDocumentation.lua`
- **ClubFinder** (81) -- `ClubFinderDocumentation.lua`
- **ClubShared** (1) -- `ClubSharedDocumentation.lua`
- **ColorOverrideConstants** (1) -- `ColorOverrideConstantsDocumentation.lua`
- **ColorOverrides** (10) -- `ColorOverridesDocumentation.lua`
- **ColorUtil** (8) -- `ColorUtilDocumentation.lua`
- **CombatAudioAlert** (17) -- `CombatAudioAlertDocumentation.lua`
- **CombatAudioAlertConstants** (1) -- `CombatAudioAlertConstantsDocumentation.lua`
- **CombatAudioAlertShared** (17) -- `CombatAudioAlertSharedDocumentation.lua`
- **CombatLog** (16) -- `CombatLogDocumentation.lua`
- **CombatLogConstants** (5) -- `CombatLogConstantsDocumentation.lua`
- **CombatLogInternal** (3) -- `CombatLogInternalDocumentation.lua`
- **CombatLogSecure** (13) -- `CombatLogSecureDocumentation.lua`
- **CombatLogShared** (2) -- `CombatLogSharedDocumentation.lua`
- **CombatText** (5) -- `CombatTextDocumentation.lua`
- **CombatTextShared** (1) -- `CombatTextSharedDocumentation.lua`
- **CommentatorFrame** (167) -- `CommentatorFrameDocumentation.lua`
- **CommentatorShared** (3) -- `CommentatorSharedDocumentation.lua`
- **CompactUnitFrames** (2) -- `CompactUnitFramesDocumentation.lua`
- **ConfigurationWarningConstants** (1) -- `ConfigurationWarningConstantsDocumentation.lua`
- **ConfigurationWarnings** (5) -- `ConfigurationWarningsDocumentation.lua`
- **Connection** (13) -- `ConnectionDocumentation.lua`
- **Console** (22) -- `ConsoleDocumentation.lua`
- **ConsoleScriptCollection** (9) -- `ConsoleScriptCollectionDocumentation.lua`
- **Container** (69) -- `ContainerDocumentation.lua`
- **ContentTracking** (24) -- `ContentTrackingDocumentation.lua`
- **ContentTrackingTypes** (9) -- `ContentTrackingTypesDocumentation.lua`
- **ContributionCollector** (26) -- `ContributionCollectorDocumentation.lua`
- **CooldownFrameConstants** (1) -- `CooldownFrameConstantsDocumentation.lua`
- **CooldownViewer** (11) -- `CooldownViewerDocumentation.lua`
- **CooldownViewerConstants** (7) -- `CooldownViewerConstantsDocumentation.lua`
- **CovenantCallings** (4) -- `CovenantCallingsDocumentation.lua`
- **CovenantCallingsConstants** (2) -- `CovenantCallingsConstantsDocumentation.lua`
- **CovenantPreview** (9) -- `CovenantPreviewDocumentation.lua`
- **CovenantSanctum** (22) -- `CovenantSanctumDocumentation.lua`
- **Covenants** (6) -- `CovenantsDocumentation.lua`
- **CovenantsConstants** (1) -- `CovenantsConstantsDocumentation.lua`
- **CraftingOrderUI** (63) -- `CraftingOrderUIDocumentation.lua`
- **CraftingOrderUIConstants** (3) -- `CraftingOrderUIConstantsDocumentation.lua`
- **CraftingOrderUIShared** (16) -- `CraftingOrderUISharedDocumentation.lua`
- **CreatureInfo** (14) -- `CreatureInfoDocumentation.lua`
- **CurrencyConstants** (10) -- `CurrencyConstantsDocumentation.lua`
- **CurrencyConstants_Mainline** (2) -- `CurrencyConstants_MainlineDocumentation.lua`
- **CurrencyInfo** (58) -- `CurrencyInfoDocumentation.lua`
- **Cursor** (6) -- `CursorDocumentation.lua`
- **CursorConst** (4) -- `CursorConstDocumentation.lua`
- **CursorUtil** (1) -- `CursorUtilDocumentation.lua`
- **CurveUtil** (6) -- `CurveUtilDocumentation.lua`
- **DamageConstants** (6) -- `DamageConstantsDocumentation.lua`
- **DamageMeter** (18) -- 4 secret -- `DamageMeterDocumentation.lua`
- **DamageMeterConstants** (7) -- `DamageMeterConstantsDocumentation.lua`
- **DateAndTime** (12) -- `DateAndTimeDocumentation.lua`
- **DeathAlert** (2) -- `DeathAlertDocumentation.lua`
- **DeathInfo** (23) -- `DeathInfoDocumentation.lua`
- **DeathRecap** (6) -- `DeathRecapDocumentation.lua`
- **DebugToggle** (10) -- `DebugToggleDocumentation.lua`
- **DelvesConstants** (7) -- `DelvesConstantsDocumentation.lua`
- **DelvesUI** (45) -- `DelvesUIDocumentation.lua`
- **DuelInfo** (6) -- `DuelInfoDocumentation.lua`
- **DungeonEncounterConstants** (7) -- `DungeonEncounterConstantsDocumentation.lua`
- **DurationUtil** (3) -- `DurationUtilDocumentation.lua`
- **DyeColorInfo** (11) -- `DyeColorInfoDocumentation.lua`
- **DyeColorInfoShared** (2) -- `DyeColorInfoSharedDocumentation.lua`
- **EditModeManager** (18) -- `EditModeManagerDocumentation.lua`
- **EditModeManagerConstants** (58) -- `EditModeManagerConstantsDocumentation.lua`
- **EditModeManagerConstants_Mainline** (1) -- `EditModeManagerConstants_MainlineDocumentation.lua`
- **EncodingUtil** (16) -- `EncodingUtilDocumentation.lua`
- **EncodingUtilConstants** (1) -- `EncodingUtilConstantsDocumentation.lua`
- **EncounterEvents** (9) -- `EncounterEventsDocumentation.lua`
- **EncounterEventsShared** (3) -- `EncounterEventsSharedDocumentation.lua`
- **EncounterInfo** (11) -- `EncounterInfoDocumentation.lua`
- **EncounterJournal** (29) -- `EncounterJournalDocumentation.lua`
- **EncounterJournalConstants** (8) -- `EncounterJournalConstantsDocumentation.lua`
- **EncounterTimeline** (50) -- 2 secret -- `EncounterTimelineDocumentation.lua`
- **EncounterTimelineConstants** (9) -- `EncounterTimelineConstantsDocumentation.lua`
- **EncounterWarnings** (13) -- `EncounterWarningsDocumentation.lua`
- **EndOfMatchUI** (7) -- `EndOfMatchUIDocumentation.lua`
- **EquipmentManager** (28) -- `EquipmentManagerDocumentation.lua`
- **EventSchedulerConstants** (1) -- `EventSchedulerConstantsDocumentation.lua`
- **EventSchedulerUI** (16) -- `EventSchedulerUIDocumentation.lua`
- **EventUtils** (3) -- `EventUtilsDocumentation.lua`
- **Expansion** (31) -- `ExpansionDocumentation.lua`
- **ExpansionConstants** (1) -- `ExpansionConstantsDocumentation.lua`
- **ExpansionInfo** (4) -- `ExpansionInfoDocumentation.lua`
- **ExpansionLevelConstants** (1) -- `ExpansionLevelConstantsDocumentation.lua`
- **ExpansionTrial** (3) -- `ExpansionTrialDocumentation.lua`
- **ExternalEventURL** (4) -- `ExternalEventURLDocumentation.lua`
- **FogOfWar** (5) -- `FogOfWarDocumentation.lua`
- **Font** (7) -- `FontDocumentation.lua`
- **FrameAPIArchaeologyDigsite** (1) -- `FrameAPIArchaeologyDigsiteDocumentation.lua`
- **FrameAPIBlob** (15) -- `FrameAPIBlobDocumentation.lua`
- **FrameAPICharacterModelBase** (25) -- `FrameAPICharacterModelBaseDocumentation.lua`
- **FrameAPICinematicModel** (19) -- `FrameAPICinematicModelDocumentation.lua`
- **FrameAPICooldown** (47) -- `FrameAPICooldownDocumentation.lua`
- **FrameAPIDressUpModel** (21) -- `FrameAPIDressUpModelDocumentation.lua`
- **FrameAPIFogOfWarFrame** (13) -- `FrameAPIFogOfWarFrameDocumentation.lua`
- **FrameAPIModelSceneFrame** (48) -- `FrameAPIModelSceneFrameDocumentation.lua`
- **FrameAPIModelSceneFrameActor** (32) -- `FrameAPIModelSceneFrameActorDocumentation.lua`
- **FrameAPIModelSceneFrameActorBase** (50) -- `FrameAPIModelSceneFrameActorBaseDocumentation.lua`
- **FrameAPIModelSceneFrameShared** (1) -- `FrameAPIModelSceneFrameSharedDocumentation.lua`
- **FrameAPINamePlate** (8) -- `FrameAPINamePlateDocumentation.lua`
- **FrameAPIQuestPOI** (4) -- `FrameAPIQuestPOIDocumentation.lua`
- **FrameAPIScenarioPOI** (3) -- `FrameAPIScenarioPOIDocumentation.lua`
- **FrameAPISimpleCheckout** (9) -- `FrameAPISimpleCheckoutDocumentation.lua`
- **FrameAPITabardModel** (7) -- `FrameAPITabardModelDocumentation.lua`
- **FrameAPITabardModelBase** (8) -- `FrameAPITabardModelBaseDocumentation.lua`
- **FrameAPITooltip** (9) -- `FrameAPITooltipDocumentation.lua`
- **FrameAPIUnitPositionFrame** (13) -- `FrameAPIUnitPositionFrameDocumentation.lua`
- **FrameScript** (37) -- `FrameScriptDocumentation.lua`
- **FriendList** (57) -- `FriendListDocumentation.lua`
- **GARRISON_FOLLOWER_TYPEConstants** (1) -- `GARRISON_FOLLOWER_TYPEConstantsDocumentation.lua`
- **GARRISON_TYPEConstants** (1) -- `GARRISON_TYPEConstantsDocumentation.lua`
- **GMTicketInfo** (9) -- `GMTicketInfoDocumentation.lua`
- **GameCursor** (20) -- `GameCursorDocumentation.lua`
- **GameError** (3) -- `GameErrorDocumentation.lua`
- **GamePad** (38) -- `GamePadDocumentation.lua`
- **GamePadConst** (1) -- `GamePadConstDocumentation.lua`
- **GameRules** (29) -- `GameRulesDocumentation.lua`
- **GameRulesConstants** (3) -- `GameRulesConstantsDocumentation.lua`
- **GameUI** (3) -- `GameUIDocumentation.lua`
- **GarrisonConstants** (23) -- `GarrisonConstantsDocumentation.lua`
- **GarrisonInfo** (126) -- `GarrisonInfoDocumentation.lua`
- **GarrisonShared** (6) -- `GarrisonSharedDocumentation.lua`
- **Glue** (4) -- `GlueDocumentation.lua`
- **GlyphInfo** (4) -- `GlyphInfoDocumentation.lua`
- **GossipConstants** (3) -- `GossipConstantsDocumentation.lua`
- **GossipInfo** (38) -- `GossipInfoDocumentation.lua`
- **GuildBank** (12) -- `GuildBankDocumentation.lua`
- **GuildConstants** (1) -- `GuildConstantsDocumentation.lua`
- **GuildInfo** (68) -- 1 secret -- `GuildInfoDocumentation.lua`
- **GuildInfoShared** (1) -- `GuildInfoSharedDocumentation.lua`
- **HeirloomInfo** (10) -- `HeirloomInfoDocumentation.lua`
- **HouseEditorUI** (13) -- `HouseEditorUIDocumentation.lua`
- **HouseExteriorConstants** (7) -- `HouseExteriorConstantsDocumentation.lua`
- **HouseExteriorUI** (33) -- `HouseExteriorUIDocumentation.lua`
- **HousingBasicModeUI** (30) -- `HousingBasicModeUIDocumentation.lua`
- **HousingCatalogConstants** (9) -- `HousingCatalogConstantsDocumentation.lua`
- **HousingCatalogSearcher** (48) -- `HousingCatalogSearcherAPIDocumentation.lua`
- **HousingCatalogUI** (52) -- `HousingCatalogUIDocumentation.lua`
- **HousingCleanupModeUI** (7) -- `HousingCleanupModeUIDocumentation.lua`
- **HousingCustomizeModeUI** (40) -- `HousingCustomizeModeUIDocumentation.lua`
- **HousingDecorShared** (3) -- `HousingDecorSharedDocumentation.lua`
- **HousingDecorUI** (40) -- `HousingDecorUIDocumentation.lua`
- **HousingExpertModeUI** (29) -- `HousingExpertModeUIDocumentation.lua`
- **HousingFixturePointFrame** (7) -- `HousingFixturePointFrameAPIDocumentation.lua`
- **HousingInspectModeUI** (8) -- `HousingInspectModeUIDocumentation.lua`
- **HousingLayoutPinFrame** (16) -- `HousingLayoutPinFrameAPIDocumentation.lua`
- **HousingLayoutUI** (54) -- `HousingLayoutUIDocumentation.lua`
- **HousingLayoutUITypes** (6) -- `HousingLayoutUITypesDocumentation.lua`
- **HousingNeighborhoodUI** (45) -- `HousingNeighborhoodUIDocumentation.lua`
- **HousingUI** (97) -- `HousingUIDocumentation.lua`
- **HousingUIShared** (13) -- `HousingUISharedDocumentation.lua`
- **ImageSharingConstants** (3) -- `ImageSharingConstantsDocumentation.lua`
- **ImmersiveInteractionManager** (4) -- `ImmersiveInteractionManagerDocumentation.lua`
- **InGameNavigation** (11) -- `InGameNavigationDocumentation.lua`
- **IncomingSummon** (4) -- `IncomingSummonDocumentation.lua`
- **Input** (29) -- `InputDocumentation.lua`
- **InputConstants** (1) -- `InputConstantsDocumentation.lua`
- **Instance** (21) -- `InstanceDocumentation.lua`
- **InstanceEncounter** (11) -- `InstanceEncounterDocumentation.lua`
- **InstanceLeaverInfo** (3) -- `InstanceLeaverInfoDocumentation.lua`
- **InterfaceFileManifest** (2) -- `InterfaceFileManifestDocumentation.lua`
- **InvasionInfo** (6) -- `InvasionInfoDocumentation.lua`
- **IslandsInfo** (2) -- `IslandsInfoDocumentation.lua`
- **IslandsQueueUI** (10) -- `IslandsQueueUIDocumentation.lua`
- **Item** (139) -- `ItemDocumentation.lua`
- **ItemConstants** (29) -- `ItemConstantsDocumentation.lua`
- **ItemConstants_Mainline** (4) -- `ItemConstants_MainlineDocumentation.lua`
- **ItemInteractionUI** (16) -- `ItemInteractionUIDocumentation.lua`
- **ItemQualities** (1) -- `ItemQualitiesDocumentation.lua`
- **ItemSocketInfo** (28) -- `ItemSocketInfoDocumentation.lua`
- **ItemText** (5) -- `ItemTextDocumentation.lua`
- **ItemUpgrade** (27) -- `ItemUpgradeDocumentation.lua`
- **KeyBindings** (21) -- `KeyBindingsDocumentation.lua`
- **LFGConstants** (10) -- `LFGConstantsDocumentation.lua`
- **LFGInfo** (55) -- `LFGInfoDocumentation.lua`
- **LFGListInfo** (73) -- 5 secret -- `LFGListInfoDocumentation.lua`
- **LegendaryCrafting** (25) -- `LegendaryCraftingDocumentation.lua`
- **LegendaryCraftingTypes** (7) -- `LegendaryCraftingTypesDocumentation.lua`
- **LevelConstants** (1) -- `LevelConstantsDocumentation.lua`
- **LevelLink** (3) -- `LevelLinkDocumentation.lua`
- **LevelSquish** (3) -- `LevelSquishDocumentation.lua`
- **LimitedInput** (3) -- `LimitedInputDocumentation.lua`
- **LiveEvent** (3) -- `LiveEventDocumentation.lua`
- **LoadingScreen** (3) -- `LoadingScreenDocumentation.lua`
- **LobbyMatchmakerInfo** (16) -- `LobbyMatchmakerInfoDocumentation.lua`
- **Locale** (7) -- `LocaleDocumentation.lua`
- **Localization** (13) -- `LocalizationDocumentation.lua`
- **LocalizationShared** (4) -- `LocalizationSharedDocumentation.lua`
- **Log** (5) -- `LogDocumentation.lua`
- **LogConstants** (1) -- `LogConstantsDocumentation.lua`
- **LogicConstants** (3) -- `LogicConstantsDocumentation.lua`
- **Loot** (40) -- `LootDocumentation.lua`
- **LootConstants** (4) -- `LootConstantsDocumentation.lua`
- **LootHistory** (16) -- `LootHistoryDocumentation.lua`
- **LootJournal** (6) -- `LootJournalDocumentation.lua`
- **LoreText** (4) -- `LoreTextDocumentation.lua`
- **LossOfControl** (13) -- 1 secret -- `LossOfControlDocumentation.lua`
- **LuaColorCurveObject** (13) -- `LuaColorCurveObjectAPIDocumentation.lua`
- **LuaCurveObject** (11) -- `LuaCurveObjectAPIDocumentation.lua`
- **LuaCurveObjectBase** (4) -- `LuaCurveObjectBaseAPIDocumentation.lua`
- **LuaCurveObjectConstants** (1) -- `LuaCurveObjectConstantsDocumentation.lua`
- **LuaDurationObject** (27) -- 8 secret -- `LuaDurationObjectAPIDocumentation.lua`
- **LuaDurationObjectShared** (1) -- `LuaDurationObjectSharedDocumentation.lua`
- **LuaTableUtil** (6) -- `LuaTableUtilDocumentation.lua`
- **MacOptions** (11) -- `MacOptionsDocumentation.lua`
- **MailInfo** (19) -- `MailInfoDocumentation.lua`
- **MajorFactions** (22) -- `MajorFactionsDocumentation.lua`
- **MajorFactionsConstants** (3) -- `MajorFactionsConstantsDocumentation.lua`
- **Map** (54) -- `MapDocumentation.lua`
- **MapConstants** (6) -- `MapConstantsDocumentation.lua`
- **MapExploration** (6) -- `MapExplorationDocumentation.lua`
- **MerchantFrame** (12) -- `MerchantFrameDocumentation.lua`
- **Minimap** (31) -- `MinimapDocumentation.lua`
- **MinimapConstants** (1) -- `MinimapConstantsDocumentation.lua`
- **MinimapFrame** (35) -- `MinimapFrameAPIDocumentation.lua`
- **MirrorTimer** (4) -- `MirrorTimerDocumentation.lua`
- **ModelAnimationShared** (1) -- `ModelAnimationSharedDocumentation.lua`
- **MoneyConstants** (1) -- `MoneyConstantsDocumentation.lua`
- **MountConstants** (3) -- `MountConstantsDocumentation.lua`
- **MountJournal** (56) -- `MountJournalDocumentation.lua`
- **Movie** (7) -- `MovieDocumentation.lua`
- **MythicPlusInfo** (30) -- `MythicPlusInfoDocumentation.lua`
- **MythicPlusInfoShared** (4) -- `MythicPlusInfoSharedDocumentation.lua`
- **NamePlate** (5) -- `NamePlateDocumentation.lua`
- **NamePlateConstants** (11) -- `NamePlateConstantsDocumentation.lua`
- **NamePlateManager** (12) -- `NamePlateManagerDocumentation.lua`
- **NeighborhoodInitiative** (33) -- `NeighborhoodInitiativeDocumentation.lua`
- **NeighborhoodInitiativesConstants** (8) -- `NeighborhoodInitiativesConstantsDocumentation.lua`
- **NewItems** (4) -- `NewItemsDocumentation.lua`
- **NumericFormatter** (2) -- `NumericFormatterAPIDocumentation.lua`
- **NumericRuleFormatter** (6) -- `NumericRuleFormatterAPIDocumentation.lua`
- **NumericRuleFormatterShared** (3) -- `NumericRuleFormatterSharedDocumentation.lua`
- **Os** (3) -- `OsDocumentation.lua`
- **PVPMgrConstants** (1) -- `PVPMgrConstantsDocumentation.lua`
- **PaperDollInfo** (43) -- `PaperDollInfoDocumentation.lua`
- **ParentalControls** (2) -- `ParentalControlsDocumentation.lua`
- **PartyConstants** (4) -- `PartyConstantsDocumentation.lua`
- **PartyInfo** (80) -- 1 secret -- `PartyInfoDocumentation.lua`
- **PartyPoseUI** (7) -- `PartyPoseUIDocumentation.lua`
- **PartyPoseUIConstants** (1) -- `PartyPoseUIConstantsDocumentation.lua`
- **Performance** (7) -- `PerformanceDocumentation.lua`
- **PerksActivities** (22) -- `PerksActivitiesDocumentation.lua`
- **PerksProgram** (46) -- `PerksProgramDocumentation.lua`
- **PerksVendorConstants** (3) -- `PerksVendorConstantsDocumentation.lua`
- **PetBattleConstants** (19) -- `PetBattleConstantsDocumentation.lua`
- **PetBattles** (6) -- `PetBattlesDocumentation.lua`
- **PetConstants** (10) -- `PetConstantsDocumentation.lua`
- **PetInfo** (20) -- `PetInfoDocumentation.lua`
- **PetJournalInfo** (43) -- `PetJournalInfoDocumentation.lua`
- **PhotoSharingUI** (17) -- `PhotoSharingUIDocumentation.lua`
- **PingConstants** (7) -- `PingConstantsDocumentation.lua`
- **PingManager** (9) -- `PingManagerDocumentation.lua`
- **PingManagerSecure** (24) -- `PingManagerSecureDocumentation.lua`
- **PingPinFrame** (1) -- `PingPinFrameAPIDocumentation.lua`
- **Platform** (2) -- `PlatformDocumentation.lua`
- **PlayerChoice** (18) -- `PlayerChoiceDocumentation.lua`
- **PlayerDataConstants** (1) -- `PlayerDataConstantsDocumentation.lua`
- **PlayerHousingConstants** (30) -- `PlayerHousingConstantsDocumentation.lua`
- **PlayerInfo** (34) -- `PlayerInfoDocumentation.lua`
- **PlayerInfoShared** (2) -- `PlayerInfoSharedDocumentation.lua`
- **PlayerInteractionManager** (10) -- `PlayerInteractionManagerDocumentation.lua`
- **PlayerInteractionManagerConstants** (1) -- `PlayerInteractionManagerConstantsDocumentation.lua`
- **PlayerLocation** (8) -- `PlayerLocationDocumentation.lua`
- **PlayerMentorship** (8) -- `PlayerMentorshipDocumentation.lua`
- **PlayerMentorshipConstants** (2) -- `PlayerMentorshipConstantsDocumentation.lua`
- **PlayerScript** (173) -- 40 secret -- `PlayerScriptDocumentation.lua`
- **Pony** (1) -- `PonyDocumentation.lua`
- **PowerTypeConstants** (4) -- `PowerTypeConstantsDocumentation.lua`
- **ProceduralSpawnConstants** (2) -- `ProceduralSpawnConstantsDocumentation.lua`
- **ProfessionConstants** (20) -- `ProfessionConstantsDocumentation.lua`
- **ProfessionSpecConstants** (9) -- `ProfessionSpecConstantsDocumentation.lua`
- **ProfessionSpecUI** (30) -- `ProfessionSpecUIDocumentation.lua`
- **PvPFactionConstants** (1) -- `PvPFactionConstantsDocumentation.lua`
- **PvpInfo** (178) -- 3 secret -- `PvpInfoDocumentation.lua`
- **PvpInfoConstants** (2) -- `PvpInfoConstantsDocumentation.lua`
- **QuestConstants** (9) -- `QuestConstantsDocumentation.lua`
- **QuestHubInfo** (2) -- `QuestHubInfoDocumentation.lua`
- **QuestInfoShared** (2) -- `QuestInfoSharedDocumentation.lua`
- **QuestInfoSystem** (9) -- `QuestInfoSystemDocumentation.lua`
- **QuestItemUse** (2) -- `QuestItemUseDocumentation.lua`
- **QuestLineInfo** (10) -- `QuestLineInfoDocumentation.lua`
- **QuestLog** (117) -- `QuestLogDocumentation.lua`
- **QuestOffer** (11) -- `QuestOfferDocumentation.lua`
- **QuestRewards** (3) -- `QuestRewardsDocumentation.lua`
- **QuestSession** (24) -- `QuestSessionDocumentation.lua`
- **QuestSessionConstants** (2) -- `QuestSessionConstantsDocumentation.lua`
- **QuestTaskInfo** (13) -- `QuestTaskInfoDocumentation.lua`
- **QueueSpecific** (1) -- `QueueSpecificDocumentation.lua`
- **RaidLocks** (4) -- `RaidLocksDocumentation.lua`
- **RaidMarkers** (9) -- `RaidMarkersDocumentation.lua`
- **RecentAllies** (26) -- `RecentAlliesDocumentation.lua`
- **RecentAlliesConstants** (2) -- `RecentAlliesConstantsDocumentation.lua`
- **RecruitAFriend** (38) -- `RecruitAFriendDocumentation.lua`
- **RecruitAFriendShared** (2) -- `RecruitAFriendSharedDocumentation.lua`
- **RemixArtifactUI** (14) -- `RemixArtifactUIDocumentation.lua`
- **RenownConstants** (2) -- `RenownConstantsDocumentation.lua`
- **ReportSystem** (15) -- `ReportSystemDocumentation.lua`
- **ReportSystemConstants** (8) -- `ReportSystemConstantsDocumentation.lua`
- **ReputationInfo** (31) -- `ReputationInfoDocumentation.lua`
- **ResearchInfo** (11) -- `ResearchInfoDocumentation.lua`
- **RestrictedActions** (10) -- `RestrictedActionsDocumentation.lua`
- **RestrictedActionsConstants** (2) -- `RestrictedActionsConstantsDocumentation.lua`
- **RolodexConstants** (4) -- `RolodexConstantsDocumentation.lua`
- **ScenarioInfo** (25) -- 1 secret -- `ScenarioInfoDocumentation.lua`
- **ScrappingMachineUI** (16) -- `ScrappingMachineUIDocumentation.lua`
- **Screen** (6) -- `ScreenDocumentation.lua`
- **ScreenLocationConstants** (1) -- `ScreenLocationConstantsDocumentation.lua`
- **ScriptWarnings** (2) -- `ScriptWarningsDocumentation.lua`
- **ScriptedAnimations** (7) -- `ScriptedAnimationsDocumentation.lua`
- **SeasonInfo** (3) -- `SeasonInfoDocumentation.lua`
- **SeasonsConstants** (1) -- `SeasonsConstantsDocumentation.lua`
- **SecondsFormatter** (34) -- `SecondsFormatterAPIDocumentation.lua`
- **SecondsFormatterShared** (3) -- `SecondsFormatterSharedDocumentation.lua`
- **SecretAspectConstants** (1) -- `SecretAspectConstantsDocumentation.lua`
- **SecretPredicate** (28) -- `SecretPredicateAPIDocumentation.lua`
- **SecretPredicates** (25) -- `SecretPredicatesDocumentation.lua`
- **SecretWrapperConstants** (1) -- `SecretWrapperConstantsDocumentation.lua`
- **SecureTransfer** (18) -- `SecureTransferDocumentation.lua`
- **SettingsUtil** (5) -- `SettingsUtilDocumentation.lua`
- **SharedScriptObjectModelLight** (1) -- `SharedScriptObjectModelLightDocumentation.lua`
- **SharedScriptObjectUnitPositionFrame** (1) -- `SharedScriptObjectUnitPositionFrameDocumentation.lua`
- **SharedTraits** (79) -- `SharedTraitsDocumentation.lua`
- **SimpleAnim** (37) -- `SimpleAnimAPIDocumentation.lua`
- **SimpleAnimAlpha** (5) -- `SimpleAnimAlphaAPIDocumentation.lua`
- **SimpleAnimFlipBook** (11) -- `SimpleAnimFlipBookAPIDocumentation.lua`
- **SimpleAnimGroup** (29) -- `SimpleAnimGroupAPIDocumentation.lua`
- **SimpleAnimPath** (6) -- `SimpleAnimPathAPIDocumentation.lua`
- **SimpleAnimRotation** (7) -- `SimpleAnimRotationAPIDocumentation.lua`
- **SimpleAnimScale** (9) -- `SimpleAnimScaleAPIDocumentation.lua`
- **SimpleAnimScaleLine** (1) -- `SimpleAnimScaleLineAPIDocumentation.lua`
- **SimpleAnimTextureCoordTranslation** (3) -- `SimpleAnimTextureCoordTranslationAPIDocumentation.lua`
- **SimpleAnimTranslation** (3) -- `SimpleAnimTranslationAPIDocumentation.lua`
- **SimpleAnimTranslationLine** (1) -- `SimpleAnimTranslationLineAPIDocumentation.lua`
- **SimpleAnimVertexColor** (5) -- `SimpleAnimVertexColorAPIDocumentation.lua`
- **SimpleAnimatableObject** (4) -- `SimpleAnimatableObjectAPIDocumentation.lua`
- **SimpleBrowser** (13) -- `SimpleBrowserAPIDocumentation.lua`
- **SimpleButton** (43) -- `SimpleButtonAPIDocumentation.lua`
- **SimpleCheckbox** (7) -- `SimpleCheckboxAPIDocumentation.lua`
- **SimpleColorSelect** (20) -- `SimpleColorSelectAPIDocumentation.lua`
- **SimpleControlPoint** (6) -- `SimpleControlPointAPIDocumentation.lua`
- **SimpleEditBox** (80) -- `SimpleEditBoxAPIDocumentation.lua`
- **SimpleFont** (25) -- `SimpleFontAPIDocumentation.lua`
- **SimpleFontString** (57) -- 9 secret -- `SimpleFontStringAPIDocumentation.lua`
- **SimpleFrame** (119) -- `SimpleFrameAPIDocumentation.lua`
- **SimpleFrameScriptObject** (11) -- `SimpleFrameScriptObjectAPIDocumentation.lua`
- **SimpleHTML** (24) -- `SimpleHTMLAPIDocumentation.lua`
- **SimpleHTMLShared** (1) -- `SimpleHTMLSharedDocumentation.lua`
- **SimpleLine** (10) -- `SimpleLineAPIDocumentation.lua`
- **SimpleMapScene** (6) -- `SimpleMapSceneAPIDocumentation.lua`
- **SimpleMaskTexture** (1) -- `SimpleMaskTextureAPIDocumentation.lua`
- **SimpleMessageFrame** (34) -- `SimpleMessageFrameAPIDocumentation.lua`
- **SimpleModel** (66) -- `SimpleModelAPIDocumentation.lua`
- **SimpleModelFFX** (5) -- `SimpleModelFFXAPIDocumentation.lua`
- **SimpleMovie** (5) -- `SimpleMovieAPIDocumentation.lua`
- **SimpleObject** (6) -- `SimpleObjectAPIDocumentation.lua`
- **SimpleOffScreenFrame** (9) -- `SimpleOffScreenFrameAPIDocumentation.lua`
- **SimpleRegion** (17) -- `SimpleRegionAPIDocumentation.lua`
- **SimpleScriptRegion** (50) -- 12 secret -- `SimpleScriptRegionAPIDocumentation.lua`
- **SimpleScriptRegionResizing** (14) -- 2 secret -- `SimpleScriptRegionResizingAPIDocumentation.lua`
- **SimpleScrollFrame** (10) -- `SimpleScrollFrameAPIDocumentation.lua`
- **SimpleSlider** (20) -- `SimpleSliderAPIDocumentation.lua`
- **SimpleStatusBar** (27) -- `SimpleStatusBarAPIDocumentation.lua`
- **SimpleStatusBarConstants** (3) -- `SimpleStatusBarConstantsDocumentation.lua`
- **SimpleTexture** (5) -- `SimpleTextureAPIDocumentation.lua`
- **SimpleTextureBase** (40) -- `SimpleTextureBaseAPIDocumentation.lua`
- **SkillInfo** (2) -- `SkillInfoDocumentation.lua`
- **SlashCommand** (3) -- `SlashCommandDocumentation.lua`
- **SocialConstants** (1) -- `SocialConstantsDocumentation.lua`
- **SocialQueue** (15) -- `SocialQueueDocumentation.lua`
- **SocialRestrictions** (12) -- `SocialRestrictionsDocumentation.lua`
- **SoftTargetConstants** (1) -- `SoftTargetConstantsDocumentation.lua`
- **Soulbinds** (59) -- `SoulbindsDocumentation.lua`
- **SoulbindsConstants** (6) -- `SoulbindsConstantsDocumentation.lua`
- **Sound** (10) -- `SoundDocumentation.lua`
- **SpecializationInfo** (42) -- `SpecializationInfoDocumentation.lua`
- **SpecializationShared** (7) -- `SpecializationSharedDocumentation.lua`
- **Spell** (72) -- 6 secret -- `SpellDocumentation.lua`
- **SpellActivationOverlay** (6) -- `SpellActivationOverlayDocumentation.lua`
- **SpellBook** (68) -- 4 secret -- `SpellBookDocumentation.lua`
- **SpellBookConstants** (3) -- `SpellBookConstantsDocumentation.lua`
- **SpellConstants** (3) -- `SpellConstantsDocumentation.lua`
- **SpellDiminishConstants** (2) -- `SpellDiminishConstantsDocumentation.lua`
- **SpellDiminishUI** (9) -- `SpellDiminishUIDocumentation.lua`
- **SpellID** (2) -- `SpellIDDocumentation.lua`
- **SpellShared** (5) -- `SpellSharedDocumentation.lua`
- **SplashScreen** (8) -- `SplashScreenDocumentation.lua`
- **SplashScreenConstants** (1) -- `SplashScreenConstantsDocumentation.lua`
- **StableInfo** (22) -- `StableInfoDocumentation.lua`
- **StorePublicUI** (4) -- `StorePublicUIDocumentation.lua`
- **Streaming** (5) -- `StreamingDocumentation.lua`
- **StringUtil** (16) -- `StringUtilDocumentation.lua`
- **SummonConstants** (1) -- `SummonConstantsDocumentation.lua`
- **SummonInfo** (8) -- `SummonInfoDocumentation.lua`
- **SuperTrackManager** (24) -- `SuperTrackManagerDocumentation.lua`
- **SuperTrackManagerShared** (2) -- `SuperTrackManagerSharedDocumentation.lua`
- **System** (28) -- `SystemDocumentation.lua`
- **SystemTime** (8) -- `SystemTimeDocumentation.lua`
- **TTSSettings** (20) -- `TTSSettingsDocumentation.lua`
- **TalentAndGlyphConstants** (1) -- `TalentAndGlyphConstantsDocumentation.lua`
- **TalentConstants** (1) -- `TalentConstantsDocumentation.lua`
- **TalkingHead** (3) -- `TalkingHeadDocumentation.lua`
- **TargetScript** (24) -- `TargetScriptDocumentation.lua`
- **TaxiMap** (11) -- `TaxiMapDocumentation.lua`
- **TextureUtils** (15) -- `TextureUtilsDocumentation.lua`
- **Threat** (3) -- `ThreatDocumentation.lua`
- **Time** (1) -- `TimeDocumentation.lua`
- **TimerConstants** (1) -- `TimerConstantsDocumentation.lua`
- **TimerunningConstants** (1) -- `TimerunningConstantsDocumentation.lua`
- **TimerunningUI** (3) -- `TimerunningUIDocumentation.lua`
- **Title** (6) -- `TitleDocumentation.lua`
- **TooltipComparison** (7) -- `TooltipComparisonDocumentation.lua`
- **TooltipConstants** (3) -- `TooltipConstantsDocumentation.lua`
- **TooltipInfo** (86) -- 6 secret -- `TooltipInfoDocumentation.lua`
- **TooltipInfoShared** (6) -- `TooltipInfoSharedDocumentation.lua`
- **Totem** (9) -- 2 secret -- `TotemDocumentation.lua`
- **ToyBoxInfo** (8) -- `ToyBoxInfoDocumentation.lua`
- **TradeInfo** (20) -- `TradeInfoDocumentation.lua`
- **TradeSkillUI** (109) -- `TradeSkillUIDocumentation.lua`
- **TradeSkillUITypes** (30) -- `TradeSkillUITypesDocumentation.lua`
- **Trainer** (6) -- `TrainerDocumentation.lua`
- **TraitConfig** (1) -- `TraitConfigDocumentation.lua`
- **TraitConstants** (21) -- `TraitConstantsDocumentation.lua`
- **TransformManipulatorConstants** (5) -- `TransformManipulatorConstantsDocumentation.lua`
- **Transmog** (28) -- `TransmogDocumentation.lua`
- **TransmogConstants** (2) -- `TransmogConstantsDocumentation.lua`
- **TransmogItems** (92) -- `TransmogItemsDocumentation.lua`
- **TransmogOutfitConstants** (26) -- `TransmogOutfitConstantsDocumentation.lua`
- **TransmogOutfitInfo** (86) -- `TransmogOutfitInfoDocumentation.lua`
- **TransmogSets** (44) -- `TransmogSetsDocumentation.lua`
- **TransmogShared** (7) -- `TransmogSharedDocumentation.lua`
- **Tutorial** (12) -- `TutorialDocumentation.lua`
- **UIAction** (2) -- `UIActionDocumentation.lua`
- **UIActionConstants** (1) -- `UIActionConstantsDocumentation.lua`
- **UIButtonShared** (1) -- `UIButtonSharedDocumentation.lua`
- **UICharacterClassConstants** (1) -- `UICharacterClassConstantsDocumentation.lua`
- **UIColor** (2) -- `UIColorDocumentation.lua`
- **UIColorShared** (1) -- `UIColorSharedDocumentation.lua`
- **UICovenantDisplayInfoConstants** (1) -- `UICovenantDisplayInfoConstantsDocumentation.lua`
- **UIEventToastConstants** (3) -- `UIEventToastConstantsDocumentation.lua`
- **UIEventToastManager** (7) -- `UIEventToastManagerDocumentation.lua`
- **UIFrameManager** (5) -- `UIFrameManagerDocumentation.lua`
- **UIGenericWidgetDisplay** (5) -- `UIGenericWidgetDisplayDocumentation.lua`
- **UIMacros** (7) -- `UIMacrosDocumentation.lua`
- **UIManager** (10) -- `UIManagerDocumentation.lua`
- **UIMapPinShared** (1) -- `UIMapPinSharedDocumentation.lua`
- **UIModelInfo** (13) -- `UIModelInfoDocumentation.lua`
- **UIModelInfoShared** (4) -- `UIModelInfoSharedDocumentation.lua`
- **UIModifiedInstance** (3) -- `UIModifiedInstanceDocumentation.lua`
- **UIShared** (4) -- `UISharedDocumentation.lua`
- **UISystemVisibilityManager** (4) -- `UISystemVisibilityManagerDocumentation.lua`
- **UITextureConstants** (2) -- `UITextureConstantsDocumentation.lua`
- **UITimer** (6) -- `UITimerDocumentation.lua`
- **UIWidgetManager** (136) -- `UIWidgetManagerDocumentation.lua`
- **UIWidgetManagerShared** (9) -- `UIWidgetManagerSharedDocumentation.lua`
- **URL** (3) -- `URLDocumentation.lua`
- **UiModelSceneConstants** (2) -- `UiModelSceneConstantsDocumentation.lua`
- **Unit** (358) -- 63 secret -- `UnitDocumentation.lua`
- **UnitAura** (38) -- 13 secret -- `UnitAuraDocumentation.lua`
- **UnitAuraShared** (3) -- `UnitAuraSharedDocumentation.lua`
- **UnitConstants** (5) -- `UnitConstantsDocumentation.lua`
- **UnitHealPredictionCalculator** (36) -- `UnitHealPredictionCalculatorAPIDocumentation.lua`
- **UnitHealPredictionCalculatorShared** (9) -- `UnitHealPredictionCalculatorSharedDocumentation.lua`
- **UnitRole** (9) -- `UnitRoleDocumentation.lua`
- **UnitSexConstants** (1) -- `UnitSexConstantsDocumentation.lua`
- **UnitShared** (2) -- `UnitSharedDocumentation.lua`
- **UserFeedback** (3) -- `UserFeedbackDocumentation.lua`
- **ValidateNameConstants** (2) -- `ValidateNameConstantsDocumentation.lua`
- **Vehicle** (11) -- `VehicleDocumentation.lua`
- **Video** (18) -- `VideoDocumentation.lua`
- **VignetteConstants** (2) -- `VignetteConstantsDocumentation.lua`
- **VignetteInfo** (10) -- `VignetteInfoDocumentation.lua`
- **VoiceChat** (136) -- 20 secret -- `VoiceChatDocumentation.lua`
- **WarCampaign** (15) -- `WarCampaignDocumentation.lua`
- **WarbandSceneInfo** (11) -- `WarbandSceneInfoDocumentation.lua`
- **WarbandSceneInfoConstants** (1) -- `WarbandSceneInfoConstantsDocumentation.lua`
- **WeeklyRewards** (30) -- `WeeklyRewardsDocumentation.lua`
- **WorldElapsedTimerConstants** (1) -- `WorldElapsedTimerConstantsDocumentation.lua`
- **WorldLootObject** (11) -- `WorldLootObjectDocumentation.lua`
- **WorldSafeLocsUIInternal** (3) -- `WorldSafeLocsUIInternalDocumentation.lua`
- **WorldStateInfo** (7) -- 2 secret -- `WorldStateInfoDocumentation.lua`
- **WowCSConstants** (1) -- `WowCSConstantsDocumentation.lua`
- **WowEntitlementInfo** (4) -- `WowEntitlementInfoDocumentation.lua`
- **WowSurvey** (4) -- `WowSurveyDocumentation.lua`
- **WowSurveyConstants** (2) -- `WowSurveyConstantsDocumentation.lua`
- **WowTokenUI** (17) -- `WowTokenUIDocumentation.lua`
- **XMLUtil** (6) -- `XMLUtilDocumentation.lua`
- **ZoneAbility** (4) -- `ZoneAbilityDocumentation.lua`
- **ZoneScript** (6) -- `ZoneScriptDocumentation.lua`
